(() => {
  ...
  function renderRecent() {
    const recentUrls = read(RECENTS_KEY);
    const items = recentUrls.map(getCardInfo).filter(Boolean);
  ...
  function renderFavoriteSection() {
    ...
    items.forEach(item => {
      const source = [...document.querySelectorAll('a.card')].find(card => card.getAttribute('href') === item.url);
  ...
  function init() {
    ...
    renderFavoriteSection();
    renderRecent();
    // 동적으로 계산기 카드가 다시 그려지는 목록에서도 즐겨찾기 버튼을 유지합니다.
    new MutationObserver(() => { setupCards(); renderFavoriteSection(); renderRecent(); })
